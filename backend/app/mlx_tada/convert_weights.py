...
def load_pytorch_weights(repo_id: str = "HumeAI/tada-1b", models_dir: str | None = None):
    """Load LLM weights from safetensors using PyTorch framework."""
...
def load_decoder_weights(models_dir: str | None = None):
    ...
    search_patterns = [
        os.path.expanduser("~/.cache/huggingface/hub/models--HumeAI--tada-codec/snapshots/*/decoder/model.safetensors")
    ]
    if models_dir:
        search_patterns.insert(0, os.path.join(models_dir, "models--HumeAI--tada-codec/snapshots/*/decoder/model.safetensors"))
    ...
    if not paths:
        raise FileNotFoundError(
            f"No decoder safetensors found in search paths: {search_patterns}. "
